...
def calculate_demographic_data(print_data=True):
    ...
    highest_earning_country = (100 * df[df['salary']=='>50K']['native-country'].value_counts()/df['native-country'].value_counts()).sort_values(ascending=False).idxmax()
  
    highest_earning_country_percentage = round(100*len(df[(df['salary']=='>50K') & (df['native-country']==highest_earning_country)]) / len(df[(df['native-country']==highest_earning_country)]), 1)
  
    # Identify the most popular occupation for those who earn >50K in India.
    top_IN_occupation = df[(df['salary']=='>50K') & (df['native-country']=='India')]['occupation'].value_counts().sort_values(ascending=False).idxmax()
